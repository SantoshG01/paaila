...
useEffect(() => {
  ...
  return (
    <header className=" top-0 z-50 w-full bg-teal-800 text-white shadow-md">
      <div className="max-w-screen-xl mx-auto flex items-center justify-between px-4 py-4 sm:px-6 lg:px-10">
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto flex-1">
          <Link href="/" className="text-2xl font-bold text-purple-500 hover:text-purple-400 transition">
            <Image src="https://res.cloudinary.com/ddgmeh112/image/upload/v1761216373/paaila-high-resolution-logo-fotor-20251023162835_ax4tsw.png" alt="Logo" width={50} height={40} className="rounded-full" />
          </Link>
        </div>

        {/* Desktop Navigation */}
        <nav className="hidden md:flex items-center gap-4 ">
          <Link href="/cart" className="relative flex items-center  hover:text-purple-500">
            <span className="absolute -top-2 -right-2 text-xs text-white bg-indigo-600 rounded-full px-1">{cartCount}</span>
            <FaCartShopping size={20} />
          </Link>

          <Link href="/products" className="relative flex items-center hover:text-indigo-300">
            Products
          </Link>
          ...
          {isLoggedIn ? (
            ...
            <>
              ...
              <button
                onClick={openLogin}
                className="px-4 py-2 text-sm bg-indigo-600 rounded  text-white hover:bg-indigo-700 transition font-medium"
              >
                Login
              </button>
            </>
            ...
            {isLoggedIn ? (
              ...
              <>
                ...
                <button
                  ...
                  className="w-full text-left rounded  bg-indigo-600 text-white px-4 py-2 font-medium hover:bg-indigo-700 transition"
                >
                  Sign Up
                </button>
              </>
